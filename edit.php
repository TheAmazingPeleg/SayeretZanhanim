					...
					<div class="container">
					  <?php
						if(isset($_POST['letter'])){
							...
							$sender = safeNoStrip($_POST['sender']);
							$rule = password($_POST['rule']);
							$stmt = $db->query("SELECT * FROM `letter` WHERE `id`='".$id."'");
							if($users['rank'] != 3 && $users['rank'] != 5){
								echo error(".דרגת ".$rank['name']." לא יכלה לערוך איגרת מפקד");
								...
								$db->query("INSERT INTO `users`(`username`, `email`, `password`, `firstName`, `lastName`, `rank`) VALUES ('".$username."','".$email."','".$password."','".$firstName."','".$lastName."','".$rank."')");
								$to = $email;
