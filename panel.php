                ...
                <main class="col-xl-9 col-lg-8 site-main">
				  <section class="about-us section" id="commander">
					<div class="container">
					  <?php
						if($users['rank'] > 3){
							if(isset($_POST['decline'])){
								...
								$driveSelect = $db->query("SELECT * FROM `rides` WHERE `id`='".$drive."' AND `status`='1'");
					  ...
					  <?php
						$selectRides = $db->query("SELECT * FROM `rides` WHERE `status`='1' AND `end`>'".$time."' ORDER BY `start` ASC");
						if($selectRides->num_rows > 0){
							while($row = $selectRides->fetch_assoc()){
								$deleteDrive = "";
								if($users['rank'] > 3){
									$deleteDrive = "<form action=\"\" class=\"row contact-form\" method=\"post\">
								...
								echo "<button type=\"button\" class=\"collapsible\"><div style=\"float: right; margin-left: 10px;\">|&nbsp;".dateCon($row['start'])."</div><div style=\"float: right\">".$row['exitLocation'].$stops1." => ".$row['destination']."</div></button>
								  <div style=\"display: none;\">
									...
									מקומות פנויים: ".$row['seatRemaining']."
									".$deleteDrive."</div>
